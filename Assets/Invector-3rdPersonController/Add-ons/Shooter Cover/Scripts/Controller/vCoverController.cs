...
{   
    ...
    {        
        ...
        protected virtual bool CalculatePath(Vector3 position)
        {
            ...
            bool valid = UnityEngine.AI.NavMesh.CalculatePath(transform.position, position, UnityEngine.AI.NavMesh.AllAreas, path) && path.GetLenght() < (inCover ? nextCoverDistance.y : maxEnterCoverDistance) && path.status == NavMeshPathStatus.PathComplete;
            return valid;
        }
        ...
        {           
            ...
            shooterInput.cc._rigidbody.linearVelocity = Vector3.zero;
        ...
        protected virtual string TargetIKPose
        {
            get
            {
                if (inCover)
                {
                    if (shooterInput.cc.isCrouching)
                    {
                        return shooterInput.IsAiming ? side == Side.left ?(isAimingInCorner?ikLeftCrouchingCornerAiming: overBarrier?ikLeftCrouchingBarrierAiming: ikLeftCrouchingAiming) :(isAimingInCorner?ikRightCrouchingCornerAiming: overBarrier ? ikRightCrouchingBarrierAiming :ikRightCrouchingAiming) :
        ...
        protected virtual void CheckOverBarrier()
        {
            ...
            var enableHipFire = Mathf.Abs(cameraAngle) < 90;
            if (enableHipFire)
            {
                shooterInput.shooterManager.hipfireShot = shooterInput.IsCrouching && (!isInLeftCornerPositionRange&&!isInRightCornerPositionRange) && shooterInput.aimConditions && useHipFireOverBarrier;
            ...
            overBarrier = useHipFireOverBarrier && enableHipFire && !shooterInput.isAimingByInput && shooterInput.isAimingByHipFire;

            overBarrierWeight = Mathf.Lerp(overBarrierWeight, overBarrier ? 1 : 0f, Time.deltaTime * (overBarrier ? 50 : 5f));
